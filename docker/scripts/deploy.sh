...
init_database() {
    ...
    # 检查是否需要创建管理员用户
    if docker-compose exec -T backend python -c "from apps.account.models import User; exit(0 if User.objects.filter(is_supper=True).exists() else 1)" 2>/dev/null; then
        warn "管理员用户已存在，跳过创建"
    else
        info "创建管理员用户..."
        ADMIN_PASSWORD=$(generate_password)
        docker-compose exec -T backend python manage.py user add -u admin -p $ADMIN_PASSWORD -n 管理员 -s
        info "管理员用户创建成功"
        info "用户名: admin"
